...
if(duplicateCollege) return res.status(400).send({ status: false, message: "Name of college already exists" });
...
let internData = await InternModel.find({ collegeId: college._id, isDeleted:false  }).select({
    _id: 1,
    email: 1,
    mobile: 1,
    name: 1,
